#!/usr/bin/env python3
"""Train ML models from real labeled data (ml_data/<model_type>.csv) or synthetic bootstrap.

Usage:
  PYTHONPATH=src python scripts/train_ml.py --status
  PYTHONPATH=src python scripts/train_ml.py                     # train all, real CSV preferred
  PYTHONPATH=src python scripts/train_ml.py --only loss_prediction
  PYTHONPATH=src python scripts/train_ml.py --csv ml_data/loss_prediction.csv --only loss_prediction
  PYTHONPATH=src python scripts/train_ml.py --export            # build CSVs from audit logs
  PYTHONPATH=src python scripts/train_ml.py --no-synthetic      # fail if no real CSV present
"""
...
def _print_status() -> None:
    ...
    for m in status["models"]:
        ...
        print(f"  {mt:<22} {label:<45} v{m['version']:<8} trained={m['is_trained']} status={m['status']}")
        if m.get("metrics"):
            top = {k: round(v, 4) for k, v in list(m["metrics"].items())[:3]}
            print(f"    metrics: {top}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train Rytera ML models (real CSV preferred, synthetic fallback).")
    ...
    parser.add_argument("--no-synthetic", action="store_true", help="Refuse synthetic data — require real CSV")
    ...
    results = train_all_models(force=True, data_root=Path(args.data_root) if args.data_root else None, allow_synthetic=not args.no_synthetic)
    for r in results:
        print(f"  trained {r.model_type.value} v{r.model_version}: {r.metrics}")
    print(f"\n  {len(results)} model(s) trained")
    return 0
