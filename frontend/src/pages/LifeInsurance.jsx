...
export default function LifeInsuranceHub({ presets, onRunDemo, onSubmit, jobs }) {
    ...
    <div className="mx-auto max-w-4xl animate-fade-in space-y-6 pb-12">
      <div>
        <nav className="flex items-center gap-1.5 text-xs text-slate-500" aria-label="Breadcrumb">
          ...
          <Link to="/insurance" className="transition hover:text-slate-300">Commercial Hub</Link>
          <span className="text-slate-700">/</span>
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500">
                    {job?.results?.commercial_coverage_name
                      ? `${job?.results?.commercial_product_name || 'Life'} · ${job.results.commercial_coverage_name}`
                      : (job?.results?.commercial_product_name || job?.results?.insurance_line || 'Life')}
                  </p>
                </div>
