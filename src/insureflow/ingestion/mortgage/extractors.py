...
def _field(
    name: str, value: str, confidence: float = 0.9, context: str = ""
) -> list[ExtractedMortgageField]:
    if not value or not str(value).strip():
        return []
    return [
        ExtractedMortgageField(
            field_name=name, value=str(value).strip(), confidence=confidence, context=context
        )
    ]
...
def extract_w2(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        "federal_tax_withheld": re.compile(
            r"2\.\s*Federal income tax withheld.*?([\d,]+\.?\d*)", re.I | re.S
        ),
        ...
        val = (
            _first(pattern, text)
            if key != "wages_box1" and key != "federal_tax_withheld"
            else _money(pattern, text)
        )
        fields[key] = _field(key, val)
    return fields
...
def extract_tax_return_1040(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        "business_income": re.compile(
            r"Business income \(Schedule C.*?\)\s+\$?([\d,]+\.?\d*)", re.I
        ),
        "taxable_income": re.compile(r"TAXABLE INCOME \(Line 15\)\s+\$?([\d,]+\.?\d*)", re.I),
    }.items():
        val = (
            _money(pattern, text)
            if any(x in key for x in ("income", "wages", "business"))
            else _first(pattern, text)
        )
        fields[key] = _field(key, val)
    return fields
...
def extract_credit_report(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    bureau = (
        "Equifax"
        if "equifax" in text.lower()
        else "Experian"
        if "experian" in text.lower()
        else "TransUnion"
    ...
    score = _first(
        re.compile(r"(?:Equifax|Experian|TransUnion)?\s*Credit Score:\s*(\d{3})", re.I), text
    )
    if not score:
    ...
    for key, pattern in {
        ...
        "utilization_rate": re.compile(
            r"(?:Utilization Rate:|Credit Utilization)\s*([\d.]+)%?", re.I
        ),
...
def extract_business_credit(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        val = (
            _money(pattern, text)
            if key in ("annual_sales", "total_outstanding")
            else _first(pattern, text)
        )
        fields[key] = _field(key, val)
    return fields
...
def extract_rent_roll(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    total_rent = sum(
        float(m.replace(",", ""))
        for m in re.findall(r"(?:Base Rent|Monthly Rent):\s*\$?([\d,]+)", text, re.I)
    )
...
def extract_form_1003(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        "monthly_income": re.compile(
            r"(?:MONTHLY|GROSS MONTHLY) (?:INCOME|BASE):?\s*\$?([\d,]+)", re.I
        ),
        ...
        val = (
            _money(pattern, text)
            if any(x in key for x in ("amount", "income", "payment"))
            else _first(pattern, text)
        )
        fields[key] = _field(key, val)
    return fields
...
def extract_title_commitment(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        "exceptions": re.compile(
            r"(?:Schedule B|Exceptions).*?(\d+)\s*(?:item|exception)", re.I | re.S
        ),
    }.items():
...
def extract_flood_cert(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        "in_special_flood_hazard": re.compile(
            r"(?:In|Within) (?:Special Flood Hazard|SFHA):\s*(Yes|No)", re.I
        ),
...
def extract_closing_disclosure(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        "closing_cash": re.compile(
            r"(?:Cash to Close|Total Closing Costs).*?\$?([\d,]+)", re.I | re.S
        ),
        "closing_date": re.compile(r"Closing Date:\s*(.+)", re.I),
    }.items():
        val = (
            _money(pattern, text)
            if any(x in key for x in ("amount", "payment", "cash"))
            else _first(pattern, text)
        )
        fields[key] = _field(key, val)
    return fields
...
def extract_hazard_insurance(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        val = (
            _money(pattern, text)
            if any(x in key for x in ("coverage", "premium", "deductible"))
            else _first(pattern, text)
        )
        fields[key] = _field(key, val)
    return fields
...
def extract_property_tax(text: str) -> dict[str, list[ExtractedMortgageField]]:
    ...
    for key, pattern in {
        ...
        "tax_amount": re.compile(
            r"(?:Total |Annual )?Tax(?:es)? (?:Due|Amount):\s*\$?([\d,]+)", re.I
        ),
        ...
        val = (
            _money(pattern, text)
            if any(x in key for x in ("value", "amount"))
            else _first(pattern, text)
        )
        fields[key] = _field(key, val)
    return fields
...
EXTRACTOR_MAP: dict[
    MortgageDocumentType, Callable[[str], dict[str, list[ExtractedMortgageField]]]
] = {
...
def extract_fields(
    doc_type: MortgageDocumentType, text: str
) -> dict[str, list[ExtractedMortgageField]]:
