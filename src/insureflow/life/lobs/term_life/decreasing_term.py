"""Decreasing Term Life — dedicated logic path (LOB 1 · Term Life).

Coverages: Mortgage Protection Term (amortization-matched), Debt-Reducing Term
(fixed % annual reduction). Face declines over the term; premium is level.
"""
...
LOGIC_PATH = "insureflow.life.lobs.term_life.decreasing_term"
...
def underwrite_decreasing_term(ctx: LifeProductContext, *, amortize: bool, reduction_rate: float) -> LobOutcome:
    label = "Mortgage Protection Term" if amortize else "Debt-Reducing Term"
    ...
    outcome.add_condition("Loan / mortgage statement required to align the declining schedule")
    ...
    # Filed-manual level economics, reshaped to the declining benefit structure.
    ...
    outcome.components = [
        ...
        RateComponent(name="declining_shape_ratio", amount=round(shape, 4), basis=f"amortized {years}yr vs level"),
    ...
    if ctx.medical.decision.value == "decline":
        outcome.eligible = False
    apply_state_filing_gate(ctx, outcome, filed_for_state=True, product_family="decreasing_term")
    return outcome
