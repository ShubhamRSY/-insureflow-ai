...
class Settings:
    ...
    # Commercial plan: pilot | desk | book | enterprise
    rytera_plan: str = os.getenv("RYTERA_PLAN", "pilot")

    # Oracle clients: live | auto (auto resolves to live when API keys are configured, misconfigured otherwise)
...
def maybe_enable_langsmith_tracing() -> bool:
    ...
    if settings.bank_mode:
        ...
        if not allow_langsmith_in_bank():
            ...
            logger.warning("LangSmith tracing disabled in BANK_MODE (prompts would leave the customer VPC). Set LANGSMITH_ALLOW_IN_BANK=true only with a bank-approved LangSmith org.")
            return False
