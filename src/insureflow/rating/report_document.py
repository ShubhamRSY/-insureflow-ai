...
def generate_memo_report_html(results: dict[str, Any], job_id: str, now: str) -> str | None:
  ...
  <div style="margin-top:16px;">
    ...
    signoff_block = uw_signoff_html(results)
    signed = bool(signoff_block)
...
{sections_html}
  ...
  <div style="margin-top:26px;padding-top:10px;border-top:1px solid #e2e8f0;text-align:center;font-size:9px;color:#94a3b8;">
    ...
    {'' if signed else '<p>It does not constitute a binder of insurance or a binding agreement.</p>'}
  </div>
...
def _fmt_signoff_date(value: str) -> str:
    raw = str(value or "")
    if not raw:
        return "—"
    try:
        return datetime.fromisoformat(raw.replace("Z", "+00:00")).strftime("%B %d, %Y at %H:%M UTC")
    except ValueError:
        return raw


def uw_signoff_html(results: dict[str, Any]) -> str:
    """Licensed-UW sign-off + validated-terms block for report/quote documents.

    Empty string when no licensed underwriter has signed off yet.
    """
    sign = results.get("uw_sign_off") or {}
    if not isinstance(sign, dict) or not str(sign.get("signed_by") or "").strip():
        return ""
    ...
    rows = f"""
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        ...
        <span style="font-weight:700;color:{action_color};">{_esc(action)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        <span style="color:#64748b;">Signed by</span>
        <span style="font-weight:600;">{_esc(sign.get("signed_by"))}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        <span style="color:#64748b;">License number</span>
        <span>{_esc(sign.get("license_number") or "—")}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        <span style="color:#64748b;">Signed at</span>
        <span>{_esc(_fmt_signoff_date(str(sign.get("signed_at") or "")))}</span>
      </div>"""

    validated = results.get("validated_terms") or {}
    if isinstance(validated, dict) and validated.get("validated_by"):
        v_premium = validated.get("indicated_premium")
        v_limit = validated.get("limit")
        v_deductible = validated.get("deductible")
        rows += f"""
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        <span style="color:#64748b;">UW-validated premium</span>
        <span style="font-weight:600;">${float(v_premium or 0):,.2f}</span>
      </div>"""
        if v_limit:
            rows += f"""
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        <span style="color:#64748b;">UW-validated limit</span><span>${float(v_limit):,.0f}</span>
      </div>"""
        if v_deductible:
            rows += f"""
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        <span style="color:#64748b;">UW-validated deductible</span><span>${float(v_deductible):,.0f}</span>
      </div>"""
        rows += f"""
      <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e2e8f0;font-size:11px;">
        <span style="color:#64748b;">Terms validated by</span>
        <span>{_esc(validated.get("validated_by"))}{(" &middot; Lic# " + _esc(validated.get("license_number"))) if validated.get("license_number") else ""}</span>
      </div>"""

    notes = str(sign.get("notes") or "").strip()
    if notes:
        rows += f"""
      <div style="padding:6px 0 0;font-size:11px;color:#475569;"><span style="color:#64748b;">UW notes:</span> {_esc(notes)}</div>"""

    return f"""
  <div style="margin-top:18px;border:1px solid #cbd5e1;border-radius:8px;padding:14px 16px;background:#f8fafc;">
    <div style="font-size:11px;font-weight:700;color:#0f172a;text-transform:uppercase;letter-spacing:0.04em;padding-bottom:8px;border-bottom:1px solid #cbd5e1;">
      Licensed Underwriter Sign-off
    </div>
    {rows}
    <div style="margin-top:10px;display:flex;justify-content:space-between;align-items:flex-end;">
      <div style="font-size:9.5px;color:#94a3b8;">Electronically recorded in the encrypted audit trail &mdash; AI recommended, licensed underwriter decided.</div>
      <div style="border-top:1.5px solid #0f172a;width:190px;text-align:center;padding-top:3px;">
        <div style="font-family:'Snell Roundhand','Segoe Script','Brush Script MT',cursive;font-size:15px;color:#0f172a;">{_esc(sign.get("signed_by"))}</div>
        <div style="font-size:8.5px;color:#64748b;">{_esc(sign.get("signed_by"))} &middot; Lic# {_esc(sign.get("license_number") or "—")}</div>
      </div>
    </div>
  </div>"""
...
"""
    ...
    footer_disclaimer = (
        ""
        if uw_signoff_block
        else "<p>It does not constitute a binder of insurance or a binding agreement.</p>"
    )
