...
@app.get("/metrics")
@limiter.exempt  # type: ignore[misc, untyped-decorator, unused-ignore]
...
@app.get("/api/uw/workbench")
def uw_workbench(
    current: TokenData = Depends(require_role(Role.VIEWER)),
) -> dict[str, Any]:
    """UW workbench inbox — pending/approved/decided cases enriched with job results."""
    from insureflow.workflow.service import WorkflowService

    org_id = current.org_id
    records = WorkflowService().store.list_records(org_id)
    ...
    return {
        ...
        "totals": {
            ...
            "done": len(done),
            "co_sign": sum(1 for r in records if r.state.value == "pending_co_sign"),
        },
    }
